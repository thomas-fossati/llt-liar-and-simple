...
NS_LOG_COMPONENT_DEFINE ("LLTLiarAndSimple");
...
main (int argc, char *argv[])
{
  ...
  greedySenderApp.Start (Seconds (1));
  ...
  greedyReceiverApp.Start (Seconds (0));

  // liar
  uint16_t dlGreedyPort2 = 5688;
  BulkSendHelper greedySender2 ("ns3::TcpSocketFactory",
                                InetSocketAddress (UEIpIface.GetAddress (0), dlGreedyPort2));
  greedySender2.SetAttribute ("MaxBytes", UintegerValue (0));
  greedySender2.SetAttribute ("SendSize", UintegerValue (1400));
  auto greedySender2App = greedySender2.Install (appServer.Get (0));
  greedySender2App.Start (Seconds (1));

  PacketSinkHelper greedyReceiver2 ("ns3::TcpSocketFactory",
                                    InetSocketAddress (Ipv4Address::GetAny (), dlGreedyPort2));
  auto greedyReceiver2App = greedyReceiver2.Install (UE.Get (0));
  greedyReceiver2App.Start (Seconds (0));
  ...
  if (markingEnabled)
    {
      pf.localPortStart = dlGreedyPort2;
      pf.localPortEnd = dlGreedyPort2;
      tft->Add (pf);
    }

  lteHelper->ActivateDedicatedEpsBearer (UEDevice, EpsBearer (EpsBearer::NGBR_VOICE_VIDEO_GAMING), tft);
